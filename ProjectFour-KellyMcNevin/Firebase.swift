...
class FirebaseModel {
    ...
    func addNote(noteString: String, date: String){
        let noteRef = FIRDatabase.database().reference(withPath: "note")
        let noteChild = noteRef.childByAutoId()
        let noteName = noteChild.child("note title")
        noteName.setValue(noteString)
        let noteDate = noteChild.child("noteDate")
